...
void InotifyWatch::SetMask(uint32_t uMask) noexcept(false) {
    ...
    if (m_wd != -1) {
        int wd = inotify_add_watch(m_pInotify->GetDescriptor(), m_path.c_str(),
                                   uMask);
        if (wd != m_wd) {
            IN_WRITE_END_NOTHROW
            throw InotifyException(IN_EXC_MSG("changing mask failed"),
                                   wd == -1 ? errno : EINVAL, this);
        }
    }
...
void InotifyWatch::SetEnabled(bool fEnabled) noexcept(false) {
    ...
    if (m_pInotify != NULL) {
        if (fEnabled) {
            m_wd = inotify_add_watch(m_pInotify->GetDescriptor(),
                                     m_path.c_str(), m_uMask);
            if (m_wd == -1) {
                IN_WRITE_END_NOTHROW
                throw InotifyException(IN_EXC_MSG("enabling watch failed"),
                                       errno, this);
            }
            ...
            if (inotify_rm_watch(m_pInotify->GetDescriptor(), m_wd) != 0) {
                IN_WRITE_END_NOTHROW
                throw InotifyException(IN_EXC_MSG("disabling watch failed"),
                                       errno, this);
            }
...
void InotifyWatch::__Disable() {
    ...
    if (!m_fEnabled) {
        ...
        throw InotifyException(
            IN_EXC_MSG("event cannot occur on disabled watch"), EINVAL, this);
    }
...
Inotify::Inotify(boost::asio::io_context &io) noexcept(false) : m_afd(io) {
    IN_LOCK_INIT
...
void Inotify::Add(InotifyWatch *pWatch) noexcept(false) {
    ...
    if (m_fd == -1) {
        IN_WRITE_END_NOTHROW
        throw InotifyException(IN_EXC_MSG("invalid file descriptor"), EBUSY,
                               this);
    }
    ...
    if (pWatch->IsEnabled()) {
        ...
        int wd = inotify_add_watch(m_fd, pWatch->GetPath().c_str(),
                                   pWatch->GetMask());

        // adding failed - go away
        if (wd == -1) {
            IN_WRITE_END_NOTHROW
            throw InotifyException(IN_EXC_MSG("adding watch failed"), errno,
                                   this);
        }
        ...
        if (pW != NULL) {
            ...
            // try to recover old watch because it may be modified - then go
            // away
            if (inotify_add_watch(m_fd, pW->GetPath().c_str(), pW->GetMask()) <
                0) {
                IN_WRITE_END_NOTHROW
                throw InotifyException(
                    IN_EXC_MSG("watch collision detected and recovery failed"),
                    errno, this);
            } else {
                ...
                throw InotifyException(
                    IN_EXC_MSG(
                        "path already watched (but defined another way)"),
                    EBUSY, this);
            }
        }
...
void Inotify::Remove(InotifyWatch *pWatch) noexcept(false) {
    ...
    if (pWatch->m_wd != -1) {
        ...
        if (inotify_rm_watch(m_fd, pWatch->m_wd) == -1) {
            IN_WRITE_END_NOTHROW
            throw InotifyException(IN_EXC_MSG("removing watch failed"), errno,
                                   this);
        }
...
void Inotify::WaitForEvents(bool fNoIntr) noexcept(false) {
    ...
    if (len == -1 && !(errno == EWOULDBLOCK || errno == EINTR))
        throw InotifyException(IN_EXC_MSG("reading events failed"), errno,
                               this);
...
bool Inotify::GetEvent(InotifyEvent *pEvt) noexcept(false) {
    if (pEvt == NULL)
        throw InotifyException(IN_EXC_MSG("null pointer to event"), EINVAL,
                               this);
...
void Inotify::SetNonBlock(bool fNonBlock) noexcept(false) {
    ...
    if (res == -1) {
        IN_WRITE_END_NOTHROW
        throw InotifyException(IN_EXC_MSG("cannot get inotify flags"), errno,
                               this);
    }
    ...
    if (fcntl(m_fd, F_SETFL, res) == -1) {
        IN_WRITE_END_NOTHROW
        throw InotifyException(IN_EXC_MSG("cannot set inotify flags"), errno,
                               this);
    }
...
std::string
Inotify::GetCapabilityPath(InotifyCapability_t cap) noexcept(false) {
