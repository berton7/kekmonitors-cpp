...
public:
   ...
   * bytes. Consider using the @ref async_read function if you need to ensure that the
   * requested amount of data is async_read before the blocking operation completes.
   *
   * @par Example
  ...
  /**
   * This function is used to async_read data from the stream socket. The function
   * call will block until one or more bytes of data has been async_read successfully,
   * or until an error occurs.
   *
   * @param buffers One or more buffers into which the data will be async_read.
   *
   * @returns The number of bytes async_read.
   *
   * @throws asio::system_error Thrown on failure. An error code of
   * asio::error::eof indicates that the connection was closed by the
   * peer.
   *
   * @note The read_some operation may not async_read all of the requested number of
   * bytes. Consider using the @ref async_read function if you need to ensure that
   * the requested amount of data is async_read before the blocking operation
   * completes.
   *
   * @par Example
   ...
   * @param ec Set to indicate what error occurred, if any.
   *
   * @returns The number of bytes async_read. Returns 0 if an error occurred.
   *
   * @note The read_some operation may not async_read all of the requested number of
   * bytes. Consider using the @ref async_read function if you need to ensure that
   * the requested amount of data is async_read before the blocking operation
  ...
  /// Start an asynchronous async_read.
  /**
   * This function is used to asynchronously async_read data from the stream socket.
   * The function call always returns immediately.
   *
   * @param buffers One or more buffers into which the data will be async_read.
   * Although the buffers object may be copied as necessary, ownership of the
   ...
   * @code void handler(
   *   const asio::error_code& error, // Result of operation.
   *   std::size_t bytes_transferred           // Number of bytes async_read.
   * ); @endcode
   * Regardless of whether the asynchronous operation completes immediately or
   * not, the handler will not be invoked from within this function. On
   * immediate completion, invocation of the handler will be performed in a
   * manner equivalent to using asio::post().
   *
   * @note The read operation may not async_read all of the requested number of bytes.
   * Consider using the @ref async_read function if you need to ensure that the
   * requested amount of data is async_read before the asynchronous operation
   * completes.
   *
   * @par Example
   * To async_read into a single data buffer use the @ref buffer function as follows:
