    ...
    {                                                                          \
        ...
        if ((res = pthread_rwlockattr_init(&attr)) != 0)                       \
            throw InotifyException(                                            \
                IN_EXC_MSG("cannot initialize lock attributes"), res, this);   \
        if ((res = pthread_rwlockattr_setkind_np(                              \
                 &attr, PTHREAD_RWLOCK_PREFER_WRITER_NP)) != 0)                \
            throw InotifyException(IN_EXC_MSG("cannot set lock kind"), res,    \
                                   this);                                      \
        if ((res = pthread_rwlock_init(&__m_lock, &attr)) != 0)                \
            throw InotifyException(IN_EXC_MSG("cannot initialize lock"), res,  \
                                   this);                                      \
...
#define IN_READ_BEGIN                                                          \
    {                                                                          \
        ...
        if (res != 0)                                                          \
            throw InotifyException(IN_EXC_MSG("locking for reading failed"),   \
                                   res, (void *)this);                         \
    }
...
#define IN_READ_END                                                            \
    {                                                                          \
        ...
        if (res != 0)                                                          \
            throw InotifyException(IN_EXC_MSG("unlocking failed"), res,        \
                                   (void *)this);                              \
    }
...
#define IN_WRITE_BEGIN                                                         \
    {                                                                          \
        ...
        if (res != 0)                                                          \
            throw InotifyException(IN_EXC_MSG("locking for writing failed"),   \
                                   res, (void *)this);                         \
    }
...
class InotifyException {
  public:
     ...
     */
    InotifyException(const std::string &rMsg = "", int iErr = 0,
                     void *pSrc = NULL)
        : m_msg(rMsg), m_err(iErr) {
        m_pSrc = pSrc;
    }
     ...
     */
    inline int GetErrorNumber() const { return m_err; }

    /// Returns the exception source.
    /**
     * \return source
     */
    inline void *GetSource() const { return m_pSrc; }
...
class InotifyEvent {
  public:
     ...
     */
    InotifyEvent() : m_uMask(0), m_uCookie(0) { m_pWatch = NULL; }
     ...
     */
    inline uint32_t GetMask() const { return m_uMask; }
     ...
     */
    inline bool IsType(uint32_t uType) const { return IsType(m_uMask, uType); }

    /// Returns the event cookie.
    /**
     * \return event cookie
     */
    inline uint32_t GetCookie() const { return m_uCookie; }

    /// Returns the event name length.
    /**
     * \return event name length
     */
    inline uint32_t GetLength() const { return (uint32_t)m_name.length(); }
     ...
     */
    inline void GetName(std::string &rName) const { rName = GetName(); }

    /// Returns the source watch.
    /**
     * \return source watch
     */
    inline InotifyWatch *GetWatch() { return m_pWatch; }
...
class InotifyWatch {
  public:
    ...
    InotifyWatch(const std::string &rPath, int32_t uMask, bool fEnabled = true)
        : m_path(rPath), m_uMask(uMask), m_wd((int32_t)-1),
          m_fEnabled(fEnabled) {
        IN_LOCK_INIT
    }

    /// Destructor.
    ~InotifyWatch() { IN_LOCK_DONE }

    /// Returns the watch descriptor.
    /**
     * \return watch descriptor; -1 for inactive watch
     */
    inline int32_t GetDescriptor() const { return m_wd; }
     ...
     * \return event mask
     */
    inline uint32_t GetMask() const { return (uint32_t)m_uMask; }
     ...
     */
    inline Inotify *GetInotify() { return m_pInotify; }
     ...
     */
    inline bool IsEnabled() const { return m_fEnabled; }
     ...
     */
    inline bool IsRecursive() const { return false; }
...
class Inotify {
  public:
     ...
     */
    Inotify(boost::asio::io_context &) noexcept(false);
     ...
     * \throw InotifyException thrown if adding failed
     */
    inline void Add(InotifyWatch &rWatch) noexcept(false) { Add(&rWatch); }
     ...
     *
     * \param[in] fNoIntr if true it re-calls the system call after a handled
     * signal
     *
     ...
     * \sa SetNonBlock()
     */
    inline int GetDescriptor() const { return m_fd; }
     ...
     */
    static void SetCapability(InotifyCapability_t cap,
     ...
     *            is set here the more physical memory may be used for the
     * inotify infrastructure.
     */
  ...
  private:
    ...
    static std::string
    GetCapabilityPath(InotifyCapability_t cap) noexcept(false);
};
